...
export function useClaimAmount() {
  ...
  // Convert wei to ETH
  const claimAmountEth = claimAmountWei 
    ? formatUnits(claimAmountWei as bigint, 18)
    : '0.00004'; // Fallback or mock value

  // Calculate current USD value
  const claimAmountUsd = ethPrice 
    ? (parseFloat(claimAmountEth) * ethPrice).toFixed(4)
    : '0.10';

  // Check if it's close to $0.10 target
  const targetUsd = 0.10;
  const deviation = ethPrice 
    ? Math.abs(parseFloat(claimAmountUsd) - targetUsd) / targetUsd
    : 0;
